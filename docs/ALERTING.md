...
- Alert Rule 1 (`P1`):
  ...
  - Threshold: `>= 10 events / 15 minutes`
  - Environment: `production`
  - Action: enviar email a owner principal.
  - Re-notify: cada 10 minutos mientras siga abierta.
- Alert Rule 2 (`P2`):
  - Condition: `event.level = error`
  - Threshold: `>= 3 events / 60 minutes`
  - Environment: `production`
  - Action: email agrupado.
  - Re-notify: cada 30 minutos.
...
2. Revisar ultimo deploy en Netlify y ultimo commit mergeado en `main`.
...
6. Aplicar fix en rama de trabajo, abrir PR hacia `main`, validar `quality` y cerrar incidente con postmortem breve.
